...
import { createProject, addTechnology, addRole } from "@/app/actions";
...
import { TextAreaEditor } from "./RichTextEditor";
...
export default function PortfolioAddForm({
  availableTechnologiesProp,
  availableRolesProp,
}: {
  availableTechnologiesProp: string[];
  availableRolesProp: string[];
}) {
  ...
  const [availableRoles, setAvailableRoles] =
    useState<string[]>(availableRolesProp);
  const form = useForm<ProjectFormData>({
    ...
    validate: {
      ...
      longDescription: (value) => {
        // Check if Tiptap content is empty or only contains default content
        if (!value) return "Long description is required";

        // Check if it's the default empty content
  ...
  const handleSubmit = async (values: ProjectFormData) => {
    ...
    try {
      ...
      const projectSlug = await createProject(cleanedValues);
